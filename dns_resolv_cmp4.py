...
sub  dig_amf_region_set_pt($$$)
{
  ...
  foreach my $amfi (@$region_set_pt_l) {
     ...
     print $f "MME $mmegi_mmec dig results from the DNS server $s:\n";
...
sub  cmp_dns1($$)
{
  ...
  my $amfi_l = get_region_set_pt_list();
  foreach my $amfi (@$amfi_l) {
     my $amfi1 = get_region_set_pt_hex($amfi);
     my $d = "${amfi1}.amfi.$d4";
     my $t1 = exec_dig1($d,$c,$s1);
     my $t2 = exec_dig1($d,$c,$s2);
     if (@$t1 ~~ @$t2) {
     ...
     my $mmegi_mmec = get_mmegi_mmec_from_amf($amfi);
     my $tmp = get_mmegi_mmec_hex($mmegi_mmec);
     $d = "$mmegi_mmec.mme.$d1";
     $t1 = exec_dig1($d,$c,$s1);
     $t2 = exec_dig1($d,$c,$s2);
